...
## install-bd: build and install bd from the beads commit go.mod pins.
## gc migrates city stores forward on first write; a bd from any other commit
## then fails every read with a schema-version mismatch (ga-87h), so the gc
## install path keeps the standalone bd in lockstep. No-op when the installed
## bd already embeds the pinned beads version.
install-bd:
	@sh scripts/install-bd-pinned.sh "$(INSTALL_DIR)"

## install: build and install gc to GOPATH/bin (same location as go install),
## plus a bd built from the same pinned beads commit (see install-bd)
install: check-self-contained install-bd
	@mkdir -p $(INSTALL_DIR)
	@set -e; \
