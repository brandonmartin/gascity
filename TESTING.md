...
shards alike — gets the same 20m per-package `go test` budget. A package's wall
...
same box, so they share one number rather than drifting onto two. Raise it on a
slow or heavily shared host:
