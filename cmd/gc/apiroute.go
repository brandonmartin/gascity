...
// controller in-process — so controllerAlive alone cannot tell them apart.
// apiClient therefore tries the standalone HTTP endpoint first and, when one
// cannot be built (the city omits an [api] port because the supervisor serves
// its API on the supervisor's own port via city-scoped routes), falls through
// to the supervisor-managed client rather than reporting controller-down.
// (gascity ga-tp7)
