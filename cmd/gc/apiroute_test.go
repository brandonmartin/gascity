...
// TestAPIClientFallsThroughToSupervisor proves the ga-tp7 fix: when the
// per-city controller socket is alive (the supervisor hosts it in-process)
// but no standalone HTTP endpoint can be built — because the city omits an
// [api] port — apiClient routes to the supervisor-managed client instead of
// returning nil ("controller-down").
func TestAPIClientFallsThroughToSupervisor(t *testing.T) {
	...
	t.Run("controller-alive-no-api-port-falls-through", func(t *testing.T) {
		t.Setenv("GC_NO_API", "")
		...
		if got := apiClient(dir); got != sentinel {
			t.Fatalf("apiClient = %p, want supervisor sentinel %p (fall-through)", got, sentinel)
		}
	})
