	...
	for idx, source := range stores {
		...
		go func() {
			...
			if openRouted, err := listBothTiersForControllerDemand(source.store, beads.ListQuery{Status: "open"}); err == nil {
				appendOpenRoutedWorkUnique(&result, &resultStores, &resultStoreRefs, excludePatrolWakeWisps(openRouted), seen, source.store, source.ref, namedIdentities)
			} else {
...
// to a named identity stays the dedicated patrol-wisp probe's responsibility
// (namedSessionPatrolWispWakeDemand), matching the namedWorkReady policy that
// also skips ephemeral assigned work. Ephemeral routed work still flows through
// case (a) unchanged.
