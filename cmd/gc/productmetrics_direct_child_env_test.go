...
func captureProductMetricsDirectChildEnv(t *testing.T, invoke func() error) []string {
	...
	// flips the productmetrics projection, ga-vxs6). Declare it as passthrough
	// so the value production code deliberately seeded is what the spy sees;
	// without this the assertion reads an empty value and mistakes the scrub
	// for missing seeding. Named by literal: importing internal/testenv outside
