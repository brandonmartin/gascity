...
// (`gc runtime request-restart`): kill the process, optionally rotate
// session_key for providers that accept --session-id, then apply
// RestartRequestPatch so the next wake observes firstStart=true and uses
// the fresh-wake conversation reset. We also update
// currently_processing_bead_id to the new anchor so the divergence check
