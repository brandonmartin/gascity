...
// and returns its city name, qualified identity, and resolved runtime name.
func namedSessionEnv() (*reconcilerTestEnv, string, string, string) {
	env := newReconcilerTestEnv()
	...
	return env, cityName, identity, runtimeName
}
...
func TestReconcileSessionBeads_RecyclesDeadNamedPhantomHoldingAssignedWork(t *testing.T) {
	env, cityName, identity, runtimeName := namedSessionEnv()
...
func TestReconcileSessionBeads_HealthyAsleepCanonicalNamedSessionNotRecycled(t *testing.T) {
	env, _, identity, runtimeName := namedSessionEnv()
