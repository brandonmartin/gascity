...
// from dispatching a polecat to redo merged work (ga-oedl).
...
//
// Go subtests make the reported name unstable across agents: ga-3k76 and ga-k8q4
// were filed as `TestX/clean_config_with_residual_files_is_a_conflict` and
// `TestY/peer_successor_cross-device_tree`, so two polecats failing different
// subtests of one function search different strings and both file. Stripping at
// the first `/` keys them together.
//
// Sibling functions in one package are the second gap: ga-3k76, ga-k8q4 and
// ga-kucr name three different `TestDisableAndPurge*` functions that share a
// single root cause (ga-vxs6), and no exact-name key groups them. Eight
// productmetrics beads landed in ~38h this way.
