...
func TestOrderFiringCurrent_LastRunLookupsRunInParallel(t *testing.T) {
	...
	if got := atomic.LoadInt32(&maxInFlight); got < 2 {
		t.Fatalf("max concurrent order-run lookups = %d, want at least 2; the fan-out is still serial", got)
	}
	// Serial would be orderCount*lookupDelay; allow generous slack for slow CI
	// while still failing a genuinely serial fan-out.
	if serial := orderCount * lookupDelay; elapsed >= serial {
		t.Fatalf("check took %s, want well under the serial cost %s", elapsed, serial)
	}
}
