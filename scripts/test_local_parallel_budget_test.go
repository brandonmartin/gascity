...
// TestLocalParallelUnitCoreBoundsPackageParallelism guards the second half of
// ga-9au. Unbounded, `go test` builds and runs up to GOMAXPROCS package
// binaries at once — on top of the outer LOCAL_TEST_JOBS shards. The
// subprocess-heavy packages then starve each other into false probe and
// condition timeouts (internal/productmetrics and internal/runtime/tmux failed
// only under the parallel load and passed in isolation). The Makefile's `test`
// target already pins -p for exactly this reason; the parallel runner, which
// is strictly more contended, must not be looser.
func TestLocalParallelUnitCoreBoundsPackageParallelism(t *testing.T) {
	command := unitCoreJobCommand(t)

	got := goTestFlagValue(t, command, "p")
	if got == "" {
		t.Fatalf("unit-core job passes no -p to go test, so package parallelism is unbounded:\n%s", command)
	}
	want := makefileTestTargetFlagValue(t, "p")
	if got != want {
		t.Fatalf("unit-core -p = %q, want %q to match the Makefile `test` target's bound", got, want)
	}
}
...
// makefileTestTargetFlagValue reads a `go test` flag from the Makefile's
// single-recipe `test` target, the tuned unsharded baseline the parallel
// runner's unit sweep is a fan-out of.
func makefileTestTargetFlagValue(t *testing.T, flag string) string {
	t.Helper()
	...
	if recipe == nil {
		t.Fatal("Makefile has no single-command `test` target to compare against")
	}
	value := goTestFlagValue(t, recipe[1], flag)
	if value == "" {
		t.Fatalf("Makefile `test` recipe passes no -%s:\n%s", flag, recipe[1])
	}
	return value
}
